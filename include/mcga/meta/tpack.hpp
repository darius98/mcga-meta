...
struct tpack_contains_t<tpack<T1, Ts...>, T>
        : tpack_contains_t<tpack<Ts...>, T> {};
...
template<class T>
struct impl_for_tpack;

template<class... Ts>
struct impl_for_tpack<tpack<Ts...>> {
    using variant_type = std::variant<Ts...>;
    using tuple_type = std::tuple<Ts...>;
};

template<class T>
using variant_for = typename impl_for_tpack<T>::variant_type;

template<class T>
using tuple_for = typename impl_for_tpack<T>::tuple_type;
